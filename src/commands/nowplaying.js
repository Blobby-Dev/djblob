...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("nowplaying")
    .setDescription("Info of current music"),
      ...
      await interaction.deferReply().catch(err => {})
        ...
        if (!queue) return interaction.followUp(`There is no song on the list yet.`).catch(err => {})
          ...
          const embed = new EmbedBuilder()
              ...
              .addFields({ name: `Video Time: **[${queue.formattedCurrentTime} / ${queue.songs[0].formattedDuration}]**`, value: ` ${':blue_circle:'.repeat(part) + '🎵' + ':red_circle:'.repeat(20 - part)}`, inline: false })
